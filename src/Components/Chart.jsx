...
const Chart = () => {
  ...
  const values = [dilScore, sosyalScore, kabaScore, inceScore, gelisimScore,
    dilScoreTreshold, sosyalScoreTreshold, kabaScoreTreshold, inceScoreTreshold, genelGelisimTreshold,
    dil20Treshold, sosyal20Treshold, kaba20Treshold, ince20Treshold,
  ...
  return (
    ...
    <VictoryChart width={500} height={400} theme={VictoryTheme.material}  animate={{ duration: 1000, onLoad: { duration: 1000, easing: 'bounce' } }}>
      <VictoryLegend x={150} y={10} orientation="horizontal" data={[
        { name: 'Skor', symbol: { fill: 'rgb(33, 68, 85)' } },
      ]} />
      <VictoryLegend x={222} y={10} orientation="horizontal" data={[
        { name: 'Hedef', symbol: { fill: 'rgb(44, 205, 185)' } },
      ]} />
      <VictoryLegend x={150} y={30} orientation="horizontal" data={[
        { name: '%20 Yaş', symbol: {fill: 'orange'}},
        { name: '%30 Yaş', symbol: {fill: 'red'}}
      ]} />
      <VictoryLegend
        x={350}
        y={10}
      ...
      {genel20Treshold === 0 ? null : <VictoryLine data={twentyTresholds} x="name" y="pv" style={{
        data: { stroke: 'orange' }
      }} />}

      {genel20Treshold === 0 ? null : <VictoryLine data={thirtyTresholds} x="name" y="pv" style={{
        data: { stroke: 'red' }
      }} />}
    </VictoryChart>
  )
